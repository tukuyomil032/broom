...
function getVisualLength(str: string): number {
	// biome-ignore lint/suspicious/noControlCharactersInRegex: ANSI escape sequence uses ESC control character
...
export function generateOptionsTable(command: Command): string {
	...
	for (const info of optionInfos) {
		...
		const truncDesc =
			desc.length > descWidth - 2
				? `${desc.substring(0, descWidth - 5)}...`
				: desc;
...
export function formatCommandHelp(command: Command): string {
	...
	if (args.length > 0) {
		...
		for (const arg of args) {
			const a = arg as ArgLike;
			const argName = a.name ?? String(arg);
			const argDesc = a.description ?? "";
