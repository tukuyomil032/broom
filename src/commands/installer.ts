...
import { exists, expandPath, formatSize, getSize } from "../utils/fs.js";
...
export async function installerCommand(
	...
	console.log(
		chalk.bold(
			`Found ${installers.length} installer files (${formatSize(totalSize)}):`,
		),
	);
	console.log();
	...
	if (errors.length === 0) {
		succeedSpinner(
			removeSpinner,
			isDryRun ? "Simulation complete" : "Removal complete",
		);
	} else {
	...
	separator("─");
	console.log(
		`  Installers ${isDryRun ? "would be " : ""}removed: ${chalk.green(removedCount)}`,
	);
	console.log(
