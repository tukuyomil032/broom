...
import { debug, debugObj, debugSection } from "../utils/debug.js";
...
export async function purgeCommand(options: PurgeOptions): Promise<void> {
	...
	printHeader(
		isDryRun
			? "🧹 Purge Project Artifacts (Dry Run)"
	...
	const selectedIds = await selectItems(
		"Select artifact types to purge:",
		targetChoices,
	);
	...
	const selectedTargets = purgeTargets.filter((t) =>
	...
	succeedSpinner(
		spinner,
		`Found ${totalCount} items (${formatSize(totalSize)})`,
	);
	...
	for (const [targetId, items] of allItems) {
		...
		console.log(
			`    Items: ${items.length}  Size: ${chalk.yellow(formatSize(targetSize))}`,
		);
		...
		for (const item of items.slice(0, 3)) {
			...
			console.log(
				`      ${chalk.dim(relativePath)} ${chalk.dim(`(${formatSize(item.size)})`)}`,
			);
		}
	...
	console.log(
		chalk.bold(isDryRun ? "🧪 Dry Run Complete" : "✨ Purge Complete"),
	);
	console.log();
	console.log(
		`  Items ${isDryRun ? "would be " : ""}removed: ${chalk.green(removedCount)}`,
	);
	console.log(
