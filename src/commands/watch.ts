...
async function listWatches(): Promise<void> {
	...
	if (watches.length === 0) {
		...
		console.log(
			chalk.dim(
				'Use "broom watch --add --path <path> --threshold 5GB" to add a watch',
			),
		);
		return;
	}
	...
	for (const watch of watches) {
		...
		console.log(
			`  Notify: ${watch.notify ? chalk.green("Yes") : chalk.red("No")}`,
		);
		console.log(
			`  Auto-clean: ${watch.autoClean ? chalk.green("Yes") : chalk.red("No")}`,
		);

		if (watch.lastNotified) {
			console.log(
				`  Last notified: ${chalk.dim(watch.lastNotified.toLocaleString())}`,
			);
		}
