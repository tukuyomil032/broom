...
import { getAllScanners, getScanner, runAllScans } from "../scanners/index.js";
import { isWhitelisted, loadConfig } from "../utils/config.js";
import {
	debug,
	debugFile,
	debugObj,
...
import { confirmRemoval, selectCategories } from "../ui/prompts.js";
...
export async function cleanCommand(options: CleanOptions): Promise<void> {
	...
	if (isDryRun) {
		console.log(
			chalk.yellow(`${ICONS.dryRun} Dry Run Mode - Preview only, no deletions`),
		);
		console.log();
	}
	...
	const scanTargets = [
		...
		{ icon: "📧", name: "Mail Downloads", path: "~/Library/Mail Downloads" },
		{
			icon: "🎧",
			name: "Podcasts Cache",
			path: "~/Library/Containers/Podcasts",
		},
		{
			icon: "🩺",
		...
		{ icon: "💾", name: "Temporary Files", path: "/var/tmp, /tmp" },
		{
			icon: "🔗",
			name: "Node Modules",
			path: "Large node_modules directories",
		},
	...
	for (const target of scanTargets) {
		console.log(
			`  ${target.icon} ${target.name.padEnd(25)} ${chalk.dim(target.path)}`,
		);
	}
	...
	const summary = await runAllScans({
		...
		onProgress: (completed, total, scanner) => {
			...
			progress.update(
				completed,
				`Scanning ${scanner.category.name}... (${completed}/${total})`,
			);
		},
	});
	...
	const riskyResults = resultsWithItems.filter(
		(r) => r.category.safetyLevel === "risky",
	);
	const safeResults = resultsWithItems.filter(
		(r) => r.category.safetyLevel !== "risky",
	);
	...
	if (!options.unsafe && riskyResults.length > 0) {
		...
		console.log(
			chalk.yellow(
				`${ICONS.warning} Skipping risky categories (use --unsafe to include):`,
			),
		);
		...
		for (const result of riskyResults) {
			console.log(
				chalk.dim(
					`  ${chalk.red("●")} ${result.category.name}: ${formatSize(result.totalSize)}`,
				),
			);
	...
	const totalItems = resultsWithItems.reduce(
	...
	const selectedItems = selectedResults.reduce(
		(sum, r) => sum + r.items.length,
		0,
	);
	...
	const cleanSpinner = createSpinner(
		isDryRun ? "Simulating cleanup..." : "Cleaning...",
	);
	...
	for (const result of selectedResults) {
		...
		debug(
			`Cleaned ${cleanResult.cleanedItems} items, freed ${formatSize(cleanResult.freedSpace)}`,
		);
	...
	succeedSpinner(
		cleanSpinner,
		isDryRun ? "Simulation complete" : "Cleanup complete",
	);

	// Print summary
	const summaryHeading = isDryRun
