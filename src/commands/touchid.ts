...
async function enableTouchId(skipConfirm: boolean): Promise<void> {
	...
	console.log(
		"You can use your fingerprint instead of typing your password.\n",
	);
	...
	try {
		...
		if (status.hasTemplate) {
			...
			if (!existsSync(backupPath)) {
				execSync(`sudo cp "${PAM_SUDO_PATH}" "${backupPath}"`, {
					stdio: "pipe",
				});
			}

			// Read current content
			const content = execSync(`sudo cat "${PAM_SUDO_PATH}"`, {
				encoding: "utf-8",
			});
			const lines = content.split("\n");
...
function showStatus(): void {
	...
	console.log(
		chalk.dim("  Template: ") +
			(status.hasTemplate ? "Available" : "Not found"),
	);
	console.log();

	if (!status.enabled) {
		console.log(
			chalk.dim("Run `broom touchid enable` to enable Touch ID for sudo.\n"),
		);
	}
}
