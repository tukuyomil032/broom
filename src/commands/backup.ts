...
async function loadMetadata(): Promise<Map<string, BackupMetadata>> {
	...
	try {
		if (exists(METADATA_FILE)) {
			...
			const data = JSON.parse(content) as Record<string, BackupMetadata>;
...
async function listBackups(): Promise<void> {
	...
	for (const backup of backups) {
		...
		console.log(
			`  Created: ${backup.timestamp.toLocaleString()} (${age} days ago)`,
		);
	...
	console.log(
		chalk.dim('Use "broom backup --clean" to remove expired backups'),
	);
}
...
async function cleanExpiredBackups(): Promise<void> {
	...
	if (removedCount > 0) {
		success(
			`Removed ${removedCount} expired backup(s), freed ${formatSize(freedSpace)}`,
		);
	} else {
...
export async function backupCommand(options: BackupOptions): Promise<void> {
	...
	console.log(
		chalk.dim(
			"Backups are automatically created when using --backup flag with clean",
		),
	);
	console.log(
		chalk.dim(`Default retention period: ${DEFAULT_RETENTION_DAYS} days`),
	);
}
