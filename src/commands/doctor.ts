...
async function checkDiskUsage(): Promise<HealthCheck> {
	try {
		const output = execSync("df -k / | tail -1 | awk '{print $2, $3, $4, $5}'")
			.toString()
			.trim();
		const [total, used, _free, percentStr] = output.split(" ");
		const percent = parseInt(percentStr, 10);

		let status: "healthy" | "warning" | "critical" = "healthy";
		const message = `Disk usage: ${percent}%`;
		let recommendation: string | undefined;

		if (percent >= 90) {
			status = "critical";
			recommendation =
				'Critically low disk space! Run "broom clean" immediately';
		} else if (percent >= 80) {
			status = "warning";
		...
		return {
			...
			message,
			recommendation,
			value: `${formatSize(parseInt(used, 10) * 1024)} / ${formatSize(parseInt(total, 10) * 1024)}`,
		};
	} catch {
		return {
			name: "Disk Usage",
	...
	for (const path of cachePaths) {
		if (exists(path)) {
			try {
				...
				const size = parseInt(output.split("\t")[0], 10) * 1024;
				totalSize += size;
			} catch {
				// Skip if cannot read
			}
...
export async function doctorCommand(options: DoctorOptions): Promise<void> {
	...
	console.log(
		chalk.dim('     Run "broom doctor --verbose" for detailed information'),
	);
}
