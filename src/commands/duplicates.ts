...
import { exists, expandPath, formatSize } from "../utils/fs.js";
 ...
 */
async function calculateSmartHash(
	filePath: string,
	fileSize: number,
): Promise<string> {
...
async function findDuplicates(
	paths: string[],
	minSize: number = 0,
	_hashAlgorithm: "md5" | "sha256" = "sha256",
	...
	return new Map(
		[...hashMap.entries()].filter(([_, files]) => files.length > 1),
	);
}
 ...
 */
async function handleDuplicatesInteractive(
	...
	for (const [_hash, files] of duplicates.entries()) {
		console.log();
		console.log(
			chalk.bold(
...
export async function duplicatesCommand(
	...
	if (options.interactive || options.delete) {
		...
		for (const [_, files] of duplicates.entries()) {
			...
			console.log(
				chalk.bold(
					`Group ${groupIndex} (${files.length} files, ${formatSize(fileSize)} each):`,
				),
			);
			console.log();
		...
		console.log(
			chalk.dim("Use --interactive to select which files to keep/delete"),
		);
	}
}
...
export function createDuplicatesCommand(): Command {
	const cmd = new Command("duplicates")
		...
		.option(
			"-i, --interactive",
			"Interactive mode to choose which files to delete",
		)
