...
import { error, printHeader, success, warning } from "../ui/output.js";
import { exists, expandPath } from "../utils/fs.js";
import { enhanceCommandHelp } from "../utils/help.js";

const LAUNCH_AGENT_PATH = expandPath(
	"~/Library/LaunchAgents/com.broom.cleanup.plist",
);
...
function generateLaunchAgent(config: {
	...
	const broomPath =
		execSync("which broom").toString().trim() || "/usr/local/bin/broom";
	...
	if (config.scanners && config.scanners.length > 0) {
		args.push(
			...
			`<string>${config.scanners.join(",")}</string>`,
		);
	}
...
async function installSchedule(options: ScheduleOptions): Promise<void> {
	...
	if (options.daily) {
		...
		if (weekday === undefined) {
			error(
				`Invalid day: ${options.day}. Use: ${Object.keys(WEEKDAYS).join(", ")}`,
			);
			return;
		}
	}
	...
	try {
		...
		console.log();
		console.log(
			chalk.dim("Cleanup will run automatically according to the schedule."),
		);
...
async function listSchedule(): Promise<void> {
	...
	if (!exists(LAUNCH_AGENT_PATH)) {
		...
		console.log(
			chalk.dim(
				'Use "broom schedule --weekly --time 09:00" to create a schedule',
			),
		);
		return;
	}
	...
	try {
		...
		const weekdayMatch = content.match(
			/<key>Weekday<\/key>\s*<integer>(\d+)<\/integer>/,
		);
		const dayMatch = content.match(
			/<key>Day<\/key>\s*<integer>(\d+)<\/integer>/,
		);
		...
		if (dayMatch) {
			...
			const dayName =
				Object.keys(WEEKDAYS).find((k) => WEEKDAYS[k] === weekday) || "Unknown";
...
export async function scheduleCommand(options: ScheduleOptions): Promise<void> {
	if (
		options.list ||
		(!options.daily && !options.weekly && !options.monthly && !options.remove)
	) {
		await listSchedule();
		return;
	}
