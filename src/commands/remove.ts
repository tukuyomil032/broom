...
function detectInstallMethod():
	...
	try {
		...
		const npmList = execSync("npm list -g --depth=0 2>/dev/null", {
			encoding: "utf-8",
		});
	...
	try {
		const pnpmList = execSync("pnpm list -g 2>/dev/null", {
			encoding: "utf-8",
		});
...
async function uninstallBroom(opts: RemoveOptions): Promise<void> {
	...
	if (installMethod === "local") {
		console.log(
			chalk.yellow(
				"broom appears to be running from a local development directory.",
			),
		);
	...
	if (existingItems.length > 0) {
		...
		for (const item of existingItems) {
			console.log(
				`  ${chalk.red("•")} ${item.description}: ${chalk.dim(item.path)}`,
			);
		}
		console.log();
	}
	...
	if (existingItems.length > 0) {
		...
		dataSpinner.succeed(
			`Removed ${removed} data ${removed === 1 ? "directory" : "directories"}`,
		);
	}
	...
	if (installMethod !== "local" && installMethod !== "unknown") {
		const uninstallSpinner = ora(
			`Uninstalling broom via ${installMethod}...`,
		).start();
	...
	console.log(
		chalk.dim(
			"Remember to remove any shell completion scripts you may have added:",
		),
	);
