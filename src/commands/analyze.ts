...
import { createSpinner, failSpinner, succeedSpinner } from "../ui/output.js";
import { expandPath, formatSize, getSize } from "../utils/fs.js";
			...
			for (const entry of entries) {
				...
				try {
					...
					if (childStats.isDirectory()) {
						// Recursively scan subdirectories
						const childInfo = await scanDirectory(
							childPath,
							currentDepth + 1,
							maxDepth,
						);
 ...
 */
function generateUsageBarBlessed(
	size: number,
	parentSize: number,
	width: number = 28,
): string {
 ...
 */
function _getChildrenAtPath(root: DirInfo, targetPath: string): DirInfo[] {
...
function _printTree(
	...
	const _maxSizeForBar = Math.max(...displayed.map((item) => item.size), 1);
	...
	for (let i = 0; i < displayed.length; i++) {
		...
		const percentage =
			parentSize > 0
				? `${((item.size / parentSize) * 100).toFixed(1).padStart(5)}%`
				: "  0.0%";
 ...
 * Get disk usage info
 */
async function _getDiskUsage(): Promise<{
	total: number;
	used: number;
	free: number;
} | null> {
...
export async function analyzeCommand(options: AnalyzeOptions): Promise<void> {
	...
	const spinner = createSpinner(
		"Scanning directory structure (initial scan)...",
	);
	...
	function displayDirectory() {
		...
		const updateInfoBox = (index: number) => {
			...
			const usageBar = generateUsageBarBlessed(
				selectedItem.size,
				currentFolder.size,
				24,
			);
			...
			// Children breakdown
			const childDirs =
				selectedItem.children?.filter((c) => c.isDirectory).length ?? 0;
			const childFiles =
				selectedItem.children?.filter((c) => !c.isDirectory).length ?? 0;
		...
		listBox.on("select item", (_item: unknown, index: number) => {
			updateInfoBox(index);
		});
