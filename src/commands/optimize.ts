...
export async function optimizeCommand(options: OptimizeOptions): Promise<void> {
	...
	printHeader(
		isDryRun ? "⚡ System Optimization (Dry Run)" : "⚡ System Optimization",
	);
	...
	tasks.forEach((task, index) => {
		...
		console.log(
			`  ${chalk.cyan(`${index + 1}.`)} ${chalk.bold(task.name)} ${sudo}`,
		);
	...
	if (options.all) {
		...
		const selectedIds = await selectItems(
			"Select optimization tasks to run:",
			taskChoices,
		);
	...
	console.log(
		chalk.dim("  - Some changes may require a restart to take effect"),
	);
	console.log(chalk.dim('  - Use "broom clean" to free up disk space'));

	if (failCount > 0) {
		console.log(
			chalk.dim("  - Failed tasks can be run manually with the commands shown"),
		);
	}
}
