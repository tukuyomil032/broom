...
import { error, info, printHeader, success, warning } from "../ui/output.js";
...
async function listConfig(): Promise<void> {
	...
	console.log(
		`  Auto confirm: ${config.autoConfirm ? chalk.green("Yes") : chalk.yellow("No")}`,
	);
	...
	switch (action) {
		...
		case "whitelist-remove":
			if (!value) {
				...
				if (index) await removeFromWhitelist(parseInt(index, 10));
			} else {
				await removeFromWhitelist(parseInt(value, 10));
			}
			break;
		...
		case "blacklist-remove":
			if (!value) {
				await listConfig();
				const index = await inputPrompt("Enter index to remove:");
				if (index) await removeFromBlacklist(parseInt(index, 10));
			} else {
				await removeFromBlacklist(parseInt(value, 10));
			}
			break;
		...
		default:
			...
			console.log(
				"  safety [level]           Set safety level (safe, moderate, risky)",
			);
...
async function interactiveConfigEdit(): Promise<void> {
	...
	while (!shouldExit) {
		...
		console.log();
		console.log(
			`  ${chalk.cyan("1")} Safety Level (Current: ${config.safetyLevel})`,
		);
		console.log(
			`  ${chalk.cyan("2")} Auto Confirm (Current: ${config.autoConfirm ? "Yes" : "No"})`,
		);
		console.log(
			`  ${chalk.cyan("3")} Dry Run (Current: ${config.dryRun ? "Yes" : "No"})`,
		);
		console.log(
			`  ${chalk.cyan("4")} Verbose (Current: ${config.verbose ? "Yes" : "No"})`,
		);
		console.log(
			`  ${chalk.cyan("5")} Manage Whitelist (${config.whitelist.length} items)`,
		);
		console.log(
			`  ${chalk.cyan("6")} Manage Blacklist (${config.blacklist.length} items)`,
		);
 ...
 */
async function configureSafetyLevel(
	...
	console.log(
		`  ${chalk.cyan("2")} Moderate - Remove common temporary files (default)`,
	);
	console.log(
		`  ${chalk.cyan("3")} Risky - Remove more aggressive including some system files`,
	);
	console.log();
...
export function createConfigCommand(): Command {
	const cmd = new Command("config")
		...
		.argument(
			"[action]",
			"Action to perform (interactive, whitelist-add, etc.)",
		)
