...
import type { AppInfo, CleanableItem, ScanResult } from "../types/index.js";
...
export async function selectApp(apps: AppInfo[]): Promise<AppInfo[]> {
	...
	try {
		return await checkbox({
			message:
				"Select applications to uninstall (space to toggle, Enter to confirm):",
			choices,
			loop: false,
		});
	} catch {
	...
	try {
		return await checkbox({
			...
			choices: choices.map((c) => ({
				name: c.description
					? `${c.name} - ${chalk.dim(c.description)}`
					: c.name,
				value: c.value,
			})),
		});
	...
	const choices = [
		{
			name: "User Cache",
			value: "userCache",
			checked: currentLocations.userCache,
		},
		{
			name: "System Cache",
			value: "systemCache",
			checked: currentLocations.systemCache,
		},
		{
			name: "System Logs",
			value: "systemLogs",
			checked: currentLocations.systemLogs,
		},
		{
			name: "User Logs",
			value: "userLogs",
			checked: currentLocations.userLogs,
		},
		{ name: "Trash", value: "trash", checked: currentLocations.trash },
		{
			name: "Downloads",
			value: "downloads",
			checked: currentLocations.downloads,
		},
		{
			name: "Browser Cache",
			value: "browserCache",
			checked: currentLocations.browserCache,
		},
		{
			name: "Development Cache",
			value: "devCache",
			checked: currentLocations.devCache,
		},
		{
			name: "Xcode Cache",
			value: "xcodeCache",
			checked: currentLocations.xcodeCache,
		},
	];
 ...
 */
export async function selectCleanupAction(): Promise<
	"remove" | "back" | "cancel"
> {
	try {
		return await select({
