...
export function printFile(item: CleanableItem, index?: number): void {
	const prefix =
		index !== undefined ? `${String(index + 1).padStart(2, " ")}.` : "  ";
...
export function printScanResult(result: ScanResult): void {
	...
	if (items.length > 0) {
		...
		console.log(
			`  ${chalk.gray(ICONS.info)} ${category.name.padEnd(35)} ${chalk.gray("Empty")}`,
		);
	}
}
...
export function printRemovalSummary(summary: RemovalSummary): void {
	...
	console.log(
		`Space freed: ${chalk.yellow(formatSize(summary.totalSizeFreed))}`,
	);
	separator();
